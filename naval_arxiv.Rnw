...
results.  One interpretation might be that a navy surrenders if it
falls more than two units behind its opponent.  The situation is not
quite isomorphic to that of tennis at deuce because the restricted
game has a pre-specified length, while a tennis game has no upper
number of points.  From figure~\ref{nnp1_rec} we conclude that
$p(n-1,n)=\frac{3n-1}{4n-2}$.
...
numerically, using the {\tt hypergeo} package~\citep{hankin2015}:
