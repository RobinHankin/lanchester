...
forces, which he styled ``red'' and ``blue'', of strength $a=a(t)$ and
$b=b(t)$ respectively:
...
properties of this system and considered British naval tactcics in the
eighteenth and nineteenth centuries in the light of his equations.
Writing in 1974, \citeauthor{karr1974} went on to give analytical
solutions to equations of this, type, with equation~\ref{lanchester}
having solution

\begin{equation}\label{lanchester}
\begin{split}
a(t) &= a_0\cosh\lambda t) -\alpha b_0\sinh\lambda t\\
a(t) &= b_0\cosh\lambda t) -\alpha^{-1}a_0\sinh\lambda t
\end{split}
\end{equation}

where $\lambda=\sqrt{ck}$, $\alpha=\sqrt{c/k}$, and $(a_0,b_0)$ are
the initial fighting strengths of the two sides.  went on to consider
a number of discrete versions of this, one of which was a
generalization of that considered here.  However,
\citeauthor{karr1974} did not solve this equation explicitly.
...
\cite{zhang2023,fox2010,lanchester1956}
\cite{zhang2023,fox2010,lanchester1956}
...
Stirling shows that this is
${\sim}\frac{27}{\sqrt{2\pi}}\left(\frac{3e}{n}\right)^n\cdot
