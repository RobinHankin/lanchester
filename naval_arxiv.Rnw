...
recurrence relation $x_{n+1}=x_n\frac{n+1}{n+3} + \frac{1}{(n+1)!}\cdot\frac{2}{n+3}$ which has solution
$x_n = \frac{2^{n+2}-(n+2)}{(n+2)!}\sim (32\pi)^{-1/2}(2/n)^{-n}n^{-5/2}$.
...
hypergeometric term is harder.  First, note that it is evaluated {\em
on} the cut line (which goes from $1+0i$ to $\infty$ along the real
axis); the hypergeometric function is defined on the cut line as being
continuous from below.  In any event, the imaginary component tends to
zero as $n\longrightarrow\infty$.  But further simplification gives
...
It is worth thinking about the hypergeometric term.  We have


$$\ft(1,-n;3/2-n;1/4)=
1+\frac{n}{(2n-3)\cdot 2}+\frac{n(n-1)}{(2n-3)(2n-5)\cdot 2^2}+
\frac{n(n-1)(n-2)}{(2n-3)(2n-5)(2n-7)\cdot 2^3}+\cdots
$$

I have a certain amount of numerical evidence to suggest
that

$$\lim_{n\longrightarrow\infty}\ft(1,-n;3/2-n;1/4)=4/3$$
