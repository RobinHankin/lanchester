...
Here I consider the following problem: navies $A,B$ deploy fleets of
$a,b$ ships respectively and engage in battle.  At any point in time,
the probability that the next ship to be sunk is one of $A$ is
$b/(a+b)$.  The winner is the first to destroy all the other fleet's
ships.  What are the probabilities of winning for general
$a,b\in\mathbb{N}$?
