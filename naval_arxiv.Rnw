...
\begin{abstract}
  ...
  In this short article I investigate an acyclic Markov chain
  described by a bivariate recurrence relation.  It has direct
  application to military conflict; the preferred interpretation is
  two fleets of battleships, each trying to sink the other.  I present
  some theoretical and numerical results and suggest lines for further
  work.
...
figure~\ref{nnp1} shows some simple results.  One interpretation might
be that a navy surrenders if it falls more than two units behind its
opponent.  The situation is not quite isomorphic to that of tennis at
...
although an algebraic proof that
