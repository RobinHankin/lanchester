...
```{r testrecur}
f <- function(n){(54*3^n-16*(n+3)*2^n+(n+3)*(n+2))/(2*factorial(n+3))}
f(1:4) - c(23/24,31/40,1/2,82/315)
```
...
```{r exactapprox}
fapprox <- function(n){27/sqrt(2*pi)*(3/n)^n*exp(n)/n^(7/2)}
f(50)/fapprox(50)
f(150)/fapprox(150)
```
...
```{r logexact}
flog <- function(n){log((54*3^n-16*(n+3)*2^n+(n+3)*(n+2)))-log(2)-lfactorial(n+3)}
glog <- function(n){log(27/sqrt(2*pi))+n+n*log(3/n)-(7/2)*log(n)}
c(flog(100),glog(100),flog(100)-glog(100))
flog(1000)  # some work to do here...
glog(1000)
```
