...
other fleet's ships; if $(r,b)$ is the initial state vector, $p(r,b)$
...
the game is considered decided if $\left|r-b\right|\geqslant 2$;
figure~\ref{nnp1} shows some results for small values of $(r,b)$.  One
...
given probability.  In general, one might hope to discover discrete
analogues of Lanchester's N-square
law~\cite{lanchester1956,lepingwell1987}.
