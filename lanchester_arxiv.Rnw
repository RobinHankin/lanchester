...
numerical or analytical solutions.  \citet{ancker1987} considered
...
reals.  His analogues to equations~\ref{defining} were
...
Here I consider a related, but different system in which the fighting
units are discrete, being destroyed according to a stochastic process.
We adopt the following recursive definition for $p(r,b)$,
$r,b\in\mathbb{N}\backslash (0,0)$:
...
@
\caption{probs\label{probrounda}}
\end{center}
\end{figure}
...
Subsequent results seem not at all easy to obtain, and have no
apparent pattern.  However, it might be reasonable to conjecture that
$p(n,b)\sim\alpha\cdot(\beta/n)^n\cdot n^{-(b+1/2)}$ for some
$\alpha,\beta$ as $n\longrightarrow\infty$.
...
such that $p(n,n+a)\geqslant\alpha$.  One would then be seeking the
minimal number of additional fighting units to secure victory with a
given probability.  Figure~\ref{logoddsratioredvsblue} goes some way
to answering this, but asymptotic results would be interesting.

Such questions give rise to a natural generalization of colonel
Blotto-type zero-sum games: how might one allocate resources among
battlefields to maximize the expected number of victories?  Further,
one might regard one's combat units as vulnerable targets rather than
offensive assets; and consider their placement from the perspective of
liability allocation games~\citep{hankin2020}.
...
\frac{\beta b}{\alpha r+\beta b}$.  This case seems not at all easy to
consider beyond producing diagrams analogous to figures~\ref{small_ab}
and~\ref{logoddsratioredvsblue} for a few special cases.  Restricting
attention to equal fighting strengths, one might observe that not only
is the eventual winner a random variable, but so too is the strength
of the remaining fleet at victory.  Investigating the probability mass
function for this might be interesting.
